...
const job = await blogAutomationRepository.createJob({ origin: 'admin_manual', topic: '[TEST] HTML title element workflow', provider: 'groq', payload: { jobType: 'generate_article', articleType: 'brief' }, idempotencyKey: key });
...
const current = await blogAutomationRepository.getJob(job.id);
console.log(JSON.stringify({ jobId: job.id, stage: current?.workflowStage, executionTarget: current?.executionTarget, published: false }));
