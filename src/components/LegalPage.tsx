...
const content: Record<LegalPageKind, LegalContent> = {
  privacy: {
    ...
    description: 'What SEOIntel stores, why it is needed, and the controls available to account owners.',
    icon: ShieldCheck,
    sections: [
      { title: 'Information we process', paragraphs: ['SEOIntel processes account details, submitted public website addresses, audit summaries, findings, page metadata, workflow notes, and operational security records.'], bullets: ['Complete raw HTML is not retained.', 'Raw IP addresses are not used for audit quotas; one-way privacy hashes are used where needed.', 'Passwords and session credentials are handled by the account provider.'] },
      ...
      { title: 'Service providers', paragraphs: ['SEOIntel currently uses Vercel for the frontend and lightweight API, Supabase for accounts and data, and Render for the audit engine. Their processing is governed by their terms and the project owner’s configuration.'] },
    ],
  },
  terms: {
    title: 'Terms of service',
    description: 'The rules for using the controlled SEOIntel Free beta.',
    icon: Scale,
    sections: [
      { title: 'Beta service', paragraphs: ['SEOIntel is currently offered as a controlled beta. Features, limits, availability, and retained data may change. No uptime, ranking, traffic, or search-performance outcome is guaranteed.'] },
      ...
      { title: 'Accounts and availability', paragraphs: ['Keep account access secure and provide accurate registration information. SEOIntel may restrict submissions, pause Free audits, or suspend abusive access to protect service availability.'] },
  ...
  'acceptable-use': {
    ...
    sections: [
      ...
      { title: 'Enforcement', paragraphs: ['SEOIntel may reject, rate-limit, cancel, or investigate submissions that threaten availability or violate this policy. Relevant security records may be retained for abuse prevention.'] },
    ],
  },
  cookies: {
    title: 'Cookie and storage notice',
    description: 'Essential browser storage used by the SEOIntel beta.',
    icon: Cookie,
    sections: [
      { title: 'Essential storage', paragraphs: ['SEOIntel uses essential account/session storage, a guest audit identifier, theme preference, and local interface preferences. These support sign-in, report ownership, fair audit limits, and accessibility preferences.'] },
      { title: 'Analytics and advertising', paragraphs: ['SEOIntel does not currently require advertising cookies. Optional analytics must be documented here and placed behind an appropriate consent control before being enabled.'] },
  ...
  contact: {
    ...
    sections: [
      ...
      { title: 'Legal owner information', paragraphs: ['Business identity, postal address, legal jurisdiction, and formal notice address require owner completion before a broader launch. SEOIntel does not publish fabricated company details.'] },
    ],
  },
};
...
export default function LegalPage({ kind }: { kind: LegalPageKind }) {
  ...
  useEffect(() => {
    ...
    document.title = `${page.title} | SEOIntel`;
