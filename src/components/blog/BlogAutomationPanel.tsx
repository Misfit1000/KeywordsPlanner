...
export default function BlogAutomationPanel({ posts, onChanged }: { posts: BlogPost[]; onChanged: () => void }) {
  ...
  const [provider, setProvider] = useState({ provider: 'NVIDIA NIM', enabled: false, configured: false, model: 'qwen/qwen3.5-122b-a10b', baseUrlHost: 'integrate.api.nvidia.com', fixtureAvailable: false });
      ...
      <div className="mt-6 rounded-lg border border-border p-4">
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="text-xs text-muted-foreground"><tr><th className="pb-3 pr-4 font-medium">Origin</th><th className="pb-3 pr-4 font-medium">Topic</th><th className="pb-3 pr-4 font-medium">State</th><th className="pb-3 font-medium">Updated</th></tr></thead>
            <tbody>{jobs.slice(0, 12).map((job) => <tr key={job.id} className="border-t border-border"><td className="py-3 pr-4 text-xs">{job.origin.replaceAll('_', ' ')}</td><td className="max-w-72 py-3 pr-4"><span className="line-clamp-2">{job.customHeadline || job.topic || 'Trend discovery'}</span>{job.error && <span className="mt-1 block text-xs text-red-600 dark:text-red-300">{job.error}</span>}</td><td className="py-3 pr-4"><StatusBadge tone={jobTone(job.state)}>{job.state.replaceAll('_', ' ')}</StatusBadge></td><td className="whitespace-nowrap py-3 text-xs text-muted-foreground">{formatDate(job.updatedAt)}</td></tr>)}</tbody>
          </table>
